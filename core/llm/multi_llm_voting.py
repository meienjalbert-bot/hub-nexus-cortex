...
async def vote(prompt: str, context: str, config_path: str = "configs/consensus_models.yaml") -> Dict[str, Any]:
    cfg = yaml.safe_load(open(config_path, "r"))
    committee = cfg.get("committee", [])
    conductor = cfg.get("conductor", {})
    async def ask(m):
        text = await generate(m["model"], f"{prompt}\n\nContext:\n{context}", m.get("max_tokens", 256), m.get("temperature", 0.2), m.get("timeout_s", 12))
        return {"role": m["role"], "model": m["model"], "answer": text}

    await prewarm([m["model"] for m in committee])
    answers: List[Dict[str, Any]] = await asyncio.gather(*[ask(m) for m in committee], return_exceptions=False)
    ...
    if conductor:
        try:
            ...
            final = await generate(conductor["model"], f"Synthétise et consolide ces réponses en une seule, fiable et concise:\n\n{joined}", conductor.get("max_tokens", 384), conductor.get("temperature", 0.2), conductor.get("timeout_s", 12))
        except Exception:
            pass

    confidence = min(1.0, 0.5 + 0.5 * (len(top.get("members", [])) / max(1, len(answers))))
